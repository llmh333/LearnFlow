...
| D17 | **D14 reversed** — Database khi deploy | Quay lại **tự host Postgres trên VPS** (container `postgres` thứ 3), bỏ Supabase | Đo thực tế sau lần deploy đầu tiên: TCP connect thuần từ VPS đến Supabase pooler (`ap-southeast-2`) mất ổn định ~300-400ms (routing quốc tế của nhà cung cấp VPS không tốt, không fix được ở tầng app) — mọi request chạm DB chậm thêm ~1s. RAM thực tế đo được lúc chạy 2 container (backend+frontend) chỉ ~300MB/~960MB, đủ dư để thêm Postgres nhẹ. Không mất backup (đã có `scripts/backup.sh`, đổi từ `pg_dump <connection-string>` sang `docker exec learnflow-postgres pg_dump`). D15 (kiến trúc 2-image + CI/CD) không đổi — chỉ thêm 1 container Postgres self-host chạy song song, không build thêm image nào. |
| D18 | **D7 reversed** — Multi-tenancy | Mọi bảng dữ liệu nghiệp vụ (`vocabulary`, `vocabulary_tag`, `review_schedule`, `review_history`, `study_session`, `ai_conversation`, `mistake`, `daily_plan`) có cột `user_id`, mọi Repository/Service/Controller lọc theo `@AuthenticationPrincipal Long userId`. `language` và `mistake_category` **vẫn giữ global** (bảng tham chiếu dùng chung, không phải dữ liệu người dùng). | Phát hiện thực tế: khi có tài khoản thứ 2, dữ liệu (từ vựng, lịch ôn SRS, mistake, daily plan...) bị dùng chung hoàn toàn giữa mọi tài khoản — `daily_plan.plan_date` còn có `UNIQUE` toàn cục khiến 2 tài khoản không thể cùng có plan trong 1 ngày. D7 (giả định "chỉ 1 user thực tế") không còn đúng. Migration `V11` **xoá sạch dữ liệu nghiệp vụ cũ** (giữ `app_user`/`language`) theo lựa chọn của người dùng, vì không thể suy ra hồi tố ai tạo cái gì. Mọi lookup theo id giờ dùng biến thể `findByIdAndUser_Id` — id không thuộc về mình trả 404 giống hệt id không tồn tại, không lộ thông tin. |
